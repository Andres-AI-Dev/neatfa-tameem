...
class CSWDController:
    ...
    def find_apriltag_visually(self, gray_image):
        ...
        try:
            ...
            # If we have a locked tag, continuously re-evaluate if we should switch
            if self.locked_tag_corners is not None and self.locked_tag_center is not None:
                ...
                # Find the current locked tag and the largest tag in view
                locked_tag = None
                locked_tag_size = 0
                largest_tag = None
                largest_tag_size = 0
                ...
                for det in detections:
                    ...
                    # Check if this is our locked tag
                    center = det['center']
                    dist = np.sqrt((center[0] - self.locked_tag_center[0])**2 +
                                 (center[1] - self.locked_tag_center[1])**2)
                    if dist < self.lock_tolerance:
                        locked_tag = det
                        locked_tag_size = size

                    # Track largest tag
                    if size > largest_tag_size:
                        largest_tag = det
                        largest_tag_size = size

                # If we found our locked tag
                if locked_tag:
                    self.lock_timeout = 0

                    # Check if there's a SIGNIFICANTLY larger tag (50% bigger threshold)
                    if largest_tag_size > locked_tag_size * 1.5:
                        # Switch to the much larger (closer) tag!
                        print(f"[SWITCH] Switching from locked tag (size: {locked_tag_size:.0f}px) to larger tag (size: {largest_tag_size:.0f}px)")
                        self.locked_tag_corners = largest_tag['lb-rb-rt-lt']
                        self.locked_tag_center = largest_tag['center']
                        self.lock_duration = 0
                        return {
                            'id': largest_tag['id'],
                            'center': largest_tag['center'],
                            'corners': largest_tag['lb-rb-rt-lt'],
                            'margin': largest_tag['margin']
                        }

                    # Otherwise, keep current lock
                    self.locked_tag_center = locked_tag['center']
                    self.locked_tag_corners = locked_tag['lb-rb-rt-lt']
                    return {
                        'id': locked_tag['id'],
                        'center': locked_tag['center'],
                        'corners': locked_tag['lb-rb-rt-lt'],
                        'margin': locked_tag['margin']
                    }
